      ...
      def has_many_with_fixes(association, options = {}, &block)
        ...
        content = with_new_form_buffer do
          template.content_tag :div, :class => "has_many #{association} #{options[:class]}" do
            object.last << template.content_tag(:h3, options[:label] || object.class.reflect_on_association(association).klass.model_name.human(:count => 1.1))
            ...
            object.last << js.html_safe
          end
        end
        object.last << content.html_safe
      end
